...
# 1) Choose a private user venv and lock reticulate to it *before* loading anything
venv <- file.path(Sys.getenv("LOCALAPPDATA"), "R", "reticulate", "asu_py311")
pyexe <- file.path(venv, "Scripts", "python.exe")

# Keep reticulate away from conda auto-detection
Sys.setenv(
  RETICULATE_MINICONDA_ENABLED = "FALSE",
  RETICULATE_AUTOCONFIGURE     = "FALSE",
  RETICULATE_PYTHON            = pyexe
)

# Create the venv if missing (uses system python; you can replace with an explicit 3.11 path if needed)
if (!file.exists(pyexe)) {
  reticulate::virtualenv_create(venv, python = "python3.11")
}
```
...
# ── FORCE our own Python BEFORE reticulate is loaded ────────────────────
# (Put this at the very top of the setup chunk)

library(reticulate)
library(processx)
py_config()  # should now show ...\asu_py311\Scripts\python.exe

# install once; on subsequent runs this will no-op
py_install(c("pandas","numpy","networkx","ortools"), pip = TRUE)
...
shiny::observeEvent(input$process, {
  ...
  if (.Platform$OS.type == "windows" && workers > 1L) {
    append_cpsat_log("[run] Windows detected; forcing workers=1 for stability.")
    workers <- 1L
  }
...
",
  ...
  # ---- 5) Locate Python from your locked venv --------------------------
  shiny::incProgress(0.05, detail = "Locating Python")
  venv  <- file.path(Sys.getenv("LOCALAPPDATA"), "R", "reticulate", "asu_py311")
  pyexe <- file.path(venv, "Scripts", "python.exe")
  pybin <- if (file.exists(pyexe)) pyexe else Sys.which("python")
  if (!nzchar(pybin)) {
    shiny::showModal(shiny::modalDialog(
      title="Python not found",
      "Could not find the asu_py311 venv or a python.exe on PATH.",
      easyClose=TRUE
    ))
    return(invisible(NULL))
  }
