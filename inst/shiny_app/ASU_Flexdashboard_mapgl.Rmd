...
# --- Reticulate + Python deps bootstrap (uv-managed env) ----------------
library(reticulate)

# Make sure Python is initialized (uses the uv env shown by py_config())
invisible(py_config())

# Ensure required modules are present in the *active* Python
req <- c("pandas", "numpy", "networkx", "ortools")

missing <- req[!vapply(req, py_module_available, logical(1))]
if (length(missing)) {
  message("Installing missing Python packages into the active env: ",
          paste(missing, collapse = ", "))
  # Installs into the current uv env (no envname needed)
  reticulate::py_install(missing, pip = TRUE)
}

# Import your CP-SAT bridge; try installed pkg path, then dev path
asu_py <- tryCatch(
  import_from_path(
    module  = "asu_cpsat",
  ...
  error = function(e) {
    # dev-mode fallback (your tree shows inst/python/asu_cpsat.py)
    import_from_path(
      module  = "asu_cpsat",
      path    = normalizePath(file.path("..","..","inst","python"),
                              winslash = "/",
                              mustWork = TRUE),
      convert = TRUE
    )
...
shiny::conditionalPanel(
  ...
  shiny::tagList(
    ...
    shiny::numericInput("cpsat_workers",   "Parallel workers",          value = max(1, parallel::detectCores()-1), min = 1, step = 1),
...
shiny::observeEvent(input$process, {
  ...
  # Determine which year to use (NE states can force 2021)
  ...
  ## 1 · Wrap the long job in withProgress -------------------------------
  shiny::withProgress(message = "Running OR-Tools CP-SAT…", value = 0, {
  dbg <- function(...) message("[CP-SAT] ", paste0(..., collapse = ""))

  # 1) Build tracts + neighbours for the selected year (this was missing)
  shiny::incProgress(0.08, detail = "Downloading Census shapefiles…")
  tract_list <- tigris::tracts(state = state(), year = selected_year) |>
    dplyr::mutate(
      row_num    = dplyr::row_number(),
      continuous = sfdep::st_contiguity(geometry)  # neighbors as integer indices (1-based)
    )
  dbg("tracts n=", nrow(tract_list))

  # Quick input checks
  shiny::incProgress(0.02, detail = "Sanity checks")
  shiny::req(!is.null(uploaded_data()))
  cols_needed <- c("GEOID","tract_ASU_unemp","tract_ASU_emp","tract_pop_cur","tract_ASU_clf","tract_ASU_urate")
  missing_cols <- setdiff(cols_needed, names(uploaded_data()))
  if (length(missing_cols))
    stop("Uploaded data missing columns: ", paste(missing_cols, collapse=", "))

  # 2) Merge the BLS data to the tracts (no geometry drop yet)
  shiny::incProgress(0.05, detail = "Merging inputs")
  data_merge_local <- tract_list |>
    dplyr::left_join(uploaded_data(), by = "GEOID")

  data_merge_df <- data_merge_local |> sf::st_drop_geometry()
  dbg("merge nrows=", nrow(data_merge_df), " ncols=", ncol(data_merge_df))

  # 3) Build Python payload
  shiny::incProgress(0.10, detail = "Building Python payload")
  df_py_r <- data_merge_df |>
    dplyr::transmute(
      geoid            = GEOID,
      tract_ASU_unemp  = as.integer(tract_ASU_unemp),
      tract_ASU_emp    = as.integer(tract_ASU_emp),
  ...
  if (any(is.na(df_py_r$tract_ASU_unemp))) stop("NA in tract_ASU_unemp after coercion")
  if (any(is.na(df_py_r$tract_ASU_emp)))   stop("NA in tract_ASU_emp after coercion")
  if (any(is.na(df_py_r$tract_pop2024)))   stop("NA in tract_pop_cur after coercion")

  df_py <- reticulate::r_to_py(df_py_r, convert = TRUE)
  dbg("df_py ok")

  # 4) Neighbors as true Python list[list[int]] (0-based)
  shiny::incProgress(0.20, detail = "Preparing neighbors")
  if (!"continuous" %in% names(tract_list)) stop("tract_list lacks 'continuous' column")
  nb_r <- lapply(tract_list$continuous, function(idx) as.integer(idx) - 1L)
  nb_r <- lapply(nb_r, function(v) if (length(v)) v else integer(0))
  dbg("nb_r length=", length(nb_r),
      "; first lens=", paste(utils::head(vapply(nb_r, length, 1L), 6), collapse=","))

  py_builtins <- reticulate::import_builtins()
  nb_py <- py_builtins$list()
  for (v in nb_r) nb_py$append(py_builtins$list(v))
  dbg("nb_py len=", reticulate::py_len(nb_py),
      "; type[0]=", if (reticulate::py_len(nb_py)>0) reticulate::py_str(reticulate::py_type(nb_py[[1]])) else "NA")

  # 5) Parameters
  shiny::incProgress(0.25, detail = "Reading params")
  ...
  workers  <- if (is.null(input$cpsat_workers)) max(1L, parallel::detectCores()-1L) else as.integer(input$cpsat_workers)
  rel_gap  <- if (!is.null(input$cpsat_relgap) && !is.na(input$cpsat_relgap)) as.numeric(input$cpsat_relgap) else NULL
  dbg(sprintf("params: tau=%.6f pop=%d max=%d tlimit=%d workers=%d rel_gap=%s",
              tau, pop_thr, max_asus, tlimit, workers, if (is.null(rel_gap)) "NULL" else as.character(rel_gap)))

  # 6) Call Python
  shiny::incProgress(0.35, detail = "Calling Python solver")
  args <- list(
    df         = df_py,
    nb         = nb_py,
    tau        = tau,
  ...
  if (!is.null(rel_gap)) args$rel_gap <- rel_gap

  out <- tryCatch(
    do.call(asu_py$build_many_asus_cpsat, args),
    error = function(e) {
      message("[CP-SAT] Python error ->")
      try(print(reticulate::py_last_error()), silent = TRUE)
      stop(e)
    }
  )
  dbg("solver returned")

  # 7) Validate + map results
  shiny::incProgress(0.80, detail = "Validating output")
  asu_id <- as.integer(reticulate::py_to_r(out$asu_id))
  dbg("asu_id length=", length(asu_id))
  if (length(asu_id) != nrow(data_merge_df)) {
    stop(sprintf("CP-SAT returned %d assignments for %d tracts.",
                 length(asu_id), nrow(data_merge_df)))
  }

  shiny::incProgress(0.90, detail = "Building results")
  data_cpsat <- data_merge_local |>
    dplyr::mutate(asunum = ifelse(asu_id < 0L, 0L, as.integer(asu_id)))

  res <<- list(
    full_data       = data_cpsat,
    full_data_reset = data_cpsat,
    asu_data        = NULL,
    asu_tracts      = NULL,
    asu_summary     = data_cpsat |>
      sf::st_drop_geometry() |>
