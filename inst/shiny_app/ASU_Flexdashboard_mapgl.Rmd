...
if (is_dev_mode) {
  ...
  source(file.path(r_dir, "drop_low_unemployment.R"))
} else {
...
# ---- drop low-unemployment controls ------------------------------------
shiny::hr(style = "margin: 0.8rem 0;")
shiny::h5("Drop low unemployment tracts")
shiny::selectInput("drop_metric", "Metric",
                   choices = c("Unemployment rate" = "rate",
                               "Unemployment level" = "unemp"),
                   selected = "rate")
shiny::numericInput("drop_percentile", "Percentile to drop",
                    value = 5, min = 0, max = 100, step = 1)
shiny::checkboxInput("allow_breaks", "Allow contiguity breaks", value = FALSE)
shiny::actionButton("drop_tracts", "Drop tracts", class = "btn-warning")
...
shiny::observeEvent(input$process, {
  ...
  # ---- end observeEvent -------------------------------------------------
shiny::observeEvent(input$drop_tracts, {
  shiny::req(full_data())
  res <- drop_lowest_percentile(full_data(),
                                percentile = as.numeric(input$drop_percentile),
                                metric = input$drop_metric,
                                allow_breaks = input$allow_breaks)
  full_data(res$full_data); asu_data(res$asu_data)
  asu_tracts(res$asu_tracts); asu_summary(res$asu_summary)
  map_geom(prep_map_data(res$full_data))
  output$asu <- shiny::renderTable(asu_summary(), digits = 3)
  st <- th_state()
  if (!is.null(st)) {
    ids <- sf::st_drop_geometry(res$full_data)[, c("GEOID", "asunum")]
    st$data_merge$asunum <- ids$asunum[match(st$data_merge$GEOID, ids$GEOID)]
    st$data_merge$asunum <- dplyr::na_if(st$data_merge$asunum, 0L)
    th_state(st)
  }
  msg <- paste(length(res$dropped), "tracts dropped")
