...
python_ready <- tryCatch(
  ...
  error = function(e) { message("[env] ", e$message); FALSE }
)

if (!python_ready) {
  message("[env] Python environment not available; run ASUbuildR::setup_asu_python()")
}
...
python_ready <- isTRUE(getOption("asu_python_ready"))
if (python_ready) {
  py_config()
} else {
  message("[env] CP-SAT features disabled (Python not available)")
}
...
# ---- algorithm picker -------------------------------------------------
algo_choices <- c("Tract Hunter" = "mine", "Simple Snake" = "orig")
if (isTRUE(getOption("asu_python_ready")))
  algo_choices <- c(algo_choices, "CP-SAT (OR-Tools)" = "cpsat")
shiny::radioButtons(
  inputId = "asu_algo",
...
if (!isTRUE(getOption("asu_python_ready")))
  shiny::p("CP-SAT solver requires Python. Run ASUbuildR::setup_asu_python() to enable.")
...
if (identical(algo_choice, "cpsat")) {
  if (!isTRUE(getOption("asu_python_ready"))) {
    shiny::showModal(shiny::modalDialog(
      ...
      "CP-SAT solver requires a working Python environment. Run ASUbuildR::setup_asu_python() and restart the app.",
      easyClose = TRUE
    ))
  ...
  shiny::withProgress(message = "Running OR-Tools CP-SAT…", value = 0, {
    ...
    py_mod_path <- system.file("python", package = "ASUbuildR")
    if (py_mod_path == "") {
...
shiny::observeEvent(input$reset, {
  ...
  full_data(full_data_reset())     # restore the snapshot

  # clear selection/highlights + UI
  selected_tracts(NULL)
  highlighted_tracts(character(0))
  shiny::updateSelectInput(session, "select_asunum", selected = "None")

  # actually refresh the map (was calling an undefined initial_map_data())
  refresh_edit_map()
})
