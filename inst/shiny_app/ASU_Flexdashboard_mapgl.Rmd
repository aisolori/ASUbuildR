...
if (identical(algo_choice, "cpsat")) {
  ...
  shiny::withProgress(message = "Running OR-Tools CP-SAT…", value = 0, {
    ...
    append_cpsat_log(sprintf("[run] params: strategy=%s tau=%.6f pop=%d max=%d limit=%ds workers=%d concurrent_asus=%d rel_gap=%s max_nodes_per_asu=%s combine_time_limit=%s",
           if (use_partitioning) "partition" else "single",
                 tau, pop_thr, max_asus, tlimit, workers, parallel_asus,
...
kwargs = dict(
    ...
    graph_cut_repair_time_limit=300,
    standalone_expansion_time_limit=1800,
    incumbent_stall_seconds=300,
    stop_flag_path=r'%s',
    skip_flag_path=r'%s'
)
REL = %s
