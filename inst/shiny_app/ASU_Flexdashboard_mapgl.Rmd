...
conditionalPanel(
  ...
  checkboxInput("join_touching",
                ...
                value = TRUE),
  actionButton("pause_process", "Pause Algorithm")
)
...
observeEvent(input$process, {
  ...
  withProgress(message = "Running ASU initialisation…", value = 0, {
    ...
    # 2 · Run the chosen ASU algorithm asynchronously -----------------
    ...
    res_promise <<- if (algo_choice == "orig") {
      future_promise({
      ...
      future_promise({
        run_tract_hunter(
          ...
          verbose       = TRUE,
          pause_fn      = function() pause_requested()
        )
      })
    }
  })  # ---- withProgress ends here  -------------------------------------

  ## 2 · When the promise resolves, store results & draw map ----------
  res_promise %...>% {
    res <- .
...
observeEvent(input$pause_process, {
