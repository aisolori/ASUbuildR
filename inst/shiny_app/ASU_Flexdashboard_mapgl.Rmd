...
shiny::conditionalPanel(
  ...
  shiny::tagList(
    ...
    shiny::numericInput("cpsat_max_nodes",     "Max tracts per ASU (optional)", value = NULL, min = 1, step = 1),
  ...
  if (identical(choice, "cpsat")) {
    ...
    shiny::HTML(
      paste0(python_status, "<p><strong>CP-SAT (OR-Tools) workflow</strong></p>
       ...
       <p>
         <strong>Max tracts per ASU (optional):</strong> leave this blank for
         unrestricted ASU sizes (default). If set, each ASU is capped at that
         many tracts while the solver builds as many separate, non-overlapping
         capped ASUs as it can. Once no more capped ASUs can be built, touching
         capped ASUs are automatically combined and re-solved (uncapped) by
         CP-SAT to improve the merged result.
       </p>
       <p>
...
if (identical(algo_choice, "cpsat")) {
  ...
  shiny::withProgress(message = "Running OR-Tools CP-SAT…", value = 0, {
    ...
    max_nodes_per_asu <- if (!is.null(input$cpsat_max_nodes) && !is.na(input$cpsat_max_nodes)) as.integer(input$cpsat_max_nodes) else NA_integer_
    append_cpsat_log(sprintf("[run] params: tau=%.6f pop=%d max=%d limit=%ds workers=%d rel_gap=%s max_nodes_per_asu=%s",
...
kwargs = dict(
    ...
    root_separator_max_size=7,

    root_separator_clause_limit=2000,
